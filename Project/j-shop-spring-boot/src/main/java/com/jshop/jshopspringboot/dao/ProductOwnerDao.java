...
public class ProductOwnerDao {
	 ...
	 * update productOwner
	 */
	public ProductOwner updateProductOwnerDao(ProductOwner productOwner) {
		//		ProductOwner existingOwner = ownerRepository.findByProductOwnerEmail(ownerEmail);
		ProductOwner existingOwner = ownerRepository.findByProductOwnerEmail(productOwner.getProductOwnerEmail());
		//		if(productOwner.getAdminVerify().equalsIgnoreCase("yes")){
		existingOwner.setProductOwnerName(productOwner.getProductOwnerName());
		//			existingOwner.setProductOwnerEmail(productOwner.getProductOwnerEmail());
		existingOwner.setProductOwnerPassword(productOwner.getProductOwnerPassword());
		existingOwner.setAdminVerify("no");
		existingOwner.setAdmin(null);
		ProductOwner updateProductOwner = ownerRepository.save(existingOwner);
		return updateProductOwner;
		//		}
		//		return null;
	}
