...
public class UserService {
	 ...
	 * updateUserService
	 */
	public ResponseStructure<User> updateUserService(User user){
		User user1= userDao.updateUserDao(user);

		if(user1!= null) {
			responseStructure.setStatusCode(HttpStatus.FOUND.value());
			responseStructure.setStatusMsg("Data Updated for the ID "+ user1.getUserId());
			responseStructure.setDescription("You can now continue your shopping");
			responseStructure.setData(user1);
		}
		else {
			responseStructure.setStatusCode(HttpStatus.NOT_FOUND.value());
			responseStructure.setStatusMsg("Given Id is not prsent in table");
			responseStructure.setDescription("check your ID and try again");
			responseStructure.setData(null);
			//throw new IdNotFoundException("Id no " +product.getProductId()+" is not present in the table");
		}
		return responseStructure;
	}
}
