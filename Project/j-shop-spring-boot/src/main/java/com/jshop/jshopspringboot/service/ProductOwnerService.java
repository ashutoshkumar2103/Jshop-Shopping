...
public class ProductOwnerService {
	...
	public ResponseStructure<ProductOwner> updateProductOwnerService(ProductOwner productOwner) {
		...
		if(productOwner2!= null) {
			...
			responseStructure.setStatusMsg("Data is updated for ID = " +productOwner2.getProductOwnerId());
			responseStructure.setDescription("You can now ask admin to verify new details inorder to login again");
			responseStructure.setData(productOwner2);
		}
		else {
			responseStructure.setStatusCode(HttpStatus.NOT_FOUND.value());
			responseStructure.setStatusMsg("Data failed to updated");
			responseStructure.setDescription("cehck again and update the correct details");
			responseStructure.setData(null);
		}
		return responseStructure;
	}
